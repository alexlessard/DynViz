...
void setup()
{
  ...
  color[] _colors = new color[] {
    #E34F3A,#00FF00,#0000FF,#50FFFF,#AF00AF,#AFAF00,#8CD503,#FF5F00,#20AF7F,#5F00FF,#B00A30, #829030
  };
  colors = new color[12];
  for (int i = 0; i< 12; i++)
  {
    int ind = round(random(0,11));
        ...
        try 
        {
          ...
          if (graphElapsed >= 50) {
            synchronized(qpsPrep)
            {
              ...
              if (qpsPrep.size() >= 20) {
                qpsHistory.add(0, (Integer)qpsPrep.get(19)/*QPS_MULTIPLIER*/);
              }
...
public class Server
{
  ...
  public void addQPS(Query q)
  {
    ...
    if (clockElapsed >= servTimeSlider.GetCurrentTime()*1000) {
      this.qpsHistory.add(0, (Integer)qps);
